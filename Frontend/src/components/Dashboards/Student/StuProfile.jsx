...
import { useStudent } from '../../../context/StudentContext'; // Ensure the context is imported
import studentProfile_Bg from '../../../assets/StudentProfile_Bg.jpg';

function StuProfile() {
    const navigate = useNavigate();
    const { student } = useStudent(); // Get student from context
...
>>>>>>> 5980ca7adef84ee31e1125f24c5a54a9213b48fa
    ...
    const Show_Meal_Poll = () => {
        // Implement the navigation to Meal Poll here
    };
    ...
    return (
        <div className="h-[100vh] bg-center bg-cover" style={{ backgroundImage: `url(${studentProfile_Bg})` }}>
                ...
                <div className="bg-gray-800 rounded-lg shadow-md p-6 max-w-3xl">
                    <div className="border-b pb-4 mb-4">
                        <div className="flex justify-between items-center p-3 bg-gray-700 rounded-lg transition duration-200 hover:bg-gray-600">
                            ...
                            <span className="text-white font-semibold text-lg">{student?.name || "Unknown"}</span> {/* Add null check */}
                        </div>
                    </div>
                </div>
            </main>
            ...
            <div>
                {/* <StuComplaints /> */}
                {/* Here dynamically the first complaint from the list must be displayed, which is going to be fetched from the database */}
            </div>
        </div>
    );
}
